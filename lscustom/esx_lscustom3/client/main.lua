...
AddEventHandler('esx_lscustom3:installMod', function()
	...
	TriggerServerEvent('esx_lscustom3:refreshOwnedVehicle', myCar)
end)
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_lscustom3:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent("esx_lscustom3:buyMod", price)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.job ~= nil and PlayerData.job.name == 'mechanic3') or Config.Ismechanic3JobOnly == false then
				for k,v in pairs(Config.Zones) do
