...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	ESX.TriggerServerCallback('esx_lscustom1:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_lscustom1:installMod')
AddEventHandler('esx_lscustom1:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_lscustom1:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_lscustom1:cancelInstallMod')
AddEventHandler('esx_lscustom1:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_lscustom1:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent("esx_lscustom1:buyMod", price)
		...
		menu.close()
		TriggerEvent('esx_lscustom1:cancelInstallMod')
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Zones) do
		--local blip = AddBlipForCoord(v.Pos.x, v.Pos.y, v.Pos.z) -- blip açmak için -- kaldırın
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.job ~= nil and PlayerData.job.name == 'mechanic1') or Config.Ismechanic1JobOnly == false then
				for k,v in pairs(Config.Zones) do
